...
gost_conf_path="/koolshare/gost/config.json"
...
function checknew()
{
    ...
    if test $checknewnum = "y";then
        Install_ct
        `killall gost`
        `/koolshare/gost -C /koolshare/gost/config.json >/dev/null 2>&1 &`
    else
        exit 0
    fi
}
...
function check_file()
{
    if test ! -d "/koolshare/gost/";then
        `mkdir /koolshare/gost/`
        `chmod -R 777 /koolshare/gost/`
    fi
}
...
function Install_ct()
{
    ...
    `mv gost /koolshare/gost/gost`
    `chmod -R 777 /koolshare/gost/gost`
    `wget --no-check-certificate https://raw.githubusercontent.com/gudk/EasyGost/master/V02gost.sh && chmod -R 777 V02gost.sh && mv V02gost.sh /koolshare/init.d/V02gost.sh`
